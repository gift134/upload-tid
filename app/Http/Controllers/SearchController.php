...
class SearchController extends Controller{
    ...
    public function search_result(Request $request, $state= null,$city=null,$zip_code = null){
        ...
        $provider_list_by_zip2 = Areaprovider::select('provider_id')->where('zip_code', '<', 0)->get();

        $providers_list = Provider::select('id','name','logo_file_name','slug','phone')->wherein('id',$provider_list_by_zip)->orWhereIn('id',$provider_list_by_zip2)->where("status","Pending")->get();

        $providers_list = Provider::select('id')->wherein('id',$provider_list_by_zip)->orWhereIn('id',$provider_list_by_zip2)->where("status","Pending")->get();

        $providers_list_id = Provider::select('id')->wherein('id',$providers_list)->where("status","Pending")->get();
        ...
        $param = Helper::seo('search_result_pg',$city,$state,$zip_code);
        ...
        ->with("type_bundle", $type_bundle)->with('meta',$param);
    }
    ...
    public function state_city(Request $request, $state){
        ...
        $param = Helper::seo('state_city_pg',$statename);

        return view('pages.frontend.home.state-city')
        ->with('state_name', $statename)
        ->with("state_cities", $state_cities)->with('meta',$param);
    }
